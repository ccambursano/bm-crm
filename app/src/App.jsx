...
function CotizacionesView({ list }) {
  ...
  const openNew = () => { setForm(empty); setModal("new"); };
  const openEdit = (q) => { setForm(q); setModal("edit"); };
  const save = () => {
  ...
  const totalAprobado = items.filter(i => i.estado === "aprobada").reduce((s, i) => s + Number(i.monto || 0), 0);
  const totalPendiente = items.filter(i => i.estado === "enviada").reduce((s, i) => s + Number(i.monto || 0), 0);

  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 10 }}>
        <div style={{ display: "flex", gap: 18, fontSize: 13, color: "#6B7280" }}>
          <span>Aprobado: <b style={{ color: "#1F7A4D" }}>{fmtMoney(totalAprobado)}</b></span>
          <span>Pendiente: <b style={{ color: "#B5651D" }}>{fmtMoney(totalPendiente)}</b></span>
        </div>
              ...
              {items.map((q) => {
                ...
                return (
                  <tr key={q.id} style={{ borderTop: "1px solid #F0F1F5" }}>
                    ...
                    <td style={{ padding: "10px 14px", fontFamily: "'JetBrains Mono', monospace", fontWeight: 600 }}>{fmtMoney(q.monto)}</td>
      ...
      {modal && (
        <Modal title={modal === "new" ? "Nueva cotización" : "Editar cotización"} onClose={() => setModal(null)}>
          ...
          <Field label="Monto (ARS)"><input type="number" style={inputStyle} value={form.monto} onChange={(e) => setForm({ ...form, monto: e.target.value })} /></Field>
          <Field label="Estado">
            <select style={inputStyle} value={form.estado} onChange={(e) => setForm({ ...form, estado: e.target.value })}>
